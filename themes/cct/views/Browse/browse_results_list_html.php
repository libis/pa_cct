		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit() && ($vn_c < $vn_hits_per_block)) {
                ...
                if ($vs_table == 'ca_objects'){
                    ...
                    if(!empty($vs_authors))
                        $vs_label_detail_link = $vs_authors . "<div class='data-line'>" . $vs_label_detail_link."</div>";

                    if(!empty($vs_printing_year))
                        $vs_label_detail_link = $vs_label_detail_link . "<div class='data-line'>" . $vs_printing_year."</div>";

                    if(!empty($vs_source_type))
                        $vs_label_detail_link = $vs_label_detail_link . "<div class='data-line'>" . $vs_source_type."</div>";

                }
                if ($vs_table == 'ca_entities'){
                    $vs_label_detail_link 	= $vs_label_detail_link . "<div class='data-line'>". $qr_res->get("{$vs_table}.type_id", array('convertCodesToDisplayText' => true))."</div>";
                }
				//libis_end
				$vs_thumbnail = "";
				$vs_type_placeholder = "";
	...
	<div class='bResultListItemCol col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}'>
      ...
      <!-- onmouseover='jQuery(\"#bResultListItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultListItemExpandedInfo{$vn_id}\").hide();' -->
		...
		<div>$vs_spacer</div>
			...
			//libis_start
            // Loding of additional items on result list should only be performed if there are more records than items to be shown per block ($vn_hits_per_block)
			...
			if($qr_res->numHits() > $vn_hits_per_block)
			    print caNavLink($this->request, _t('Next %1', $vn_hits_per_block), 'jscroll-next', '*', '*', '*', array('s' => $vn_start + $vn_hits_per_block, 'key' => $vs_browse_key, 'view' => $vs_current_view, '_advanced' => 1));
			//libis_end
		}
?>
